...
/// <summary>
/// Timed plate:
/// - Có occupant (Player collider thường hoặc SwapBlock) -> ON, chìm xuống, Ring hiện FULL.
/// - Rời plate -> start countdown từ duration cố định; Ring giảm dần (fillAmount), plate trồi lên dần.
/// - Countdown tiếp tục chạy qua SHIFT (không reset). Khi quay lại đúng world, hiển thị đúng remaining.
/// - Ring chỉ ẩn khi idle chưa kích hoạt hoặc hết giờ.
/// 
/// Notes:
/// - Không rely vào DOTween cho countdown (vì tween hay bị Kill khi SHIFT). Plate position + ring fill sync theo Time.time.
/// - SwapBlock có thể "latch" điều kiện xuyên SHIFT (xem PlateBase2D.keepSwapBlockConditionAcrossWorlds).
/// </summary>
public class TimedPlate2D : PlateBase2D
{
    ...
    private void ApplyRingSorting()
    {
        ...
        // Root World Space Canvas vẫn set sorting được (không cần "Override Sorting" trong inspector)
    ...
    {
        // Never render ring when plate world is inactive.
        if (!activeInWorld)
        {
            SetRingVisible(false);
            return;
        ...
        // Ring show khi:
        // - occupied (HasOccupant) hoặc latch swapblock
        // - counting
        // Ring hide khi:
        // - idle chưa kích hoạt (armed=false)
        // - expired
        ...
        {
            // armed nhưng không counting: nếu đang occupied/latch thì show, còn lại hide (trường hợp hiếm)
